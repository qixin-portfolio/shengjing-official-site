...
const consultPaths = [
  {
    title: "预约量房",
    desc: "填写需求和联系方式，我们安排量房时间。量房后给出初步方案和报价建议。",
    items: [
      "服务区域：山西吕梁交城县及周边",
      "建议提前预约，避免跑空",
      "量房后 3 个工作日内出初步方案",
  ...
  {
    title: "微信咨询",
    desc: "添加微信咨询装修问题，发送户型图可以更快给出建议。",
    note: "二维码待客服确认后上线",
    icon: "wechat",
  },
  {
    title: "电话咨询",
    desc: "电话咨询请通过微信或到店预约，我们会在确认联系方式后更新。",
    note: "具体号码请以线下门店或客服确认为准",
    icon: "phone",
  },
];
...
const prepChecklist = [
  {
    title: "基本资料",
    icon: "doc",
    items: ["户型图（开发商给的有尺寸版）", "房屋现状照片（毛坯/老房）", "小区名称和地址"],
  },
  {
    title: "需求清单",
    icon: "list",
    items: ["家庭成员和居住需求", "风格倾向（可发参考图）", "收纳重点和特殊需求"],
  },
  {
    title: "预算范围",
    icon: "wallet",
    items: ["总预算区间（不必精确）", "哪些项目优先投入", "哪些可以省"],
  },
  {
    title: "时间安排",
    icon: "clock",
    items: ["期望开工时间", "期望入住时间", "是否需要临时住处"],
  },
];
    ...
    case "wechat":
      return (
        <svg className={common} viewBox="0 0 24 24" fill="currentColor">
          <path d="M8.5 4C4.91 4 2 6.46 2 9.5c0 1.61.83 3.06 2.17 4.06L3.5 16l2.5-1.5c.8.25 1.65.38 2.5.38.34 0 .67-.02 1-.06-.32-.6-.5-1.27-.5-1.97C9 10.69 11.69 8.5 15 8.5c.34 0 .67.03 1 .08C15.66 5.94 12.39 4 8.5 4z" />
        </svg>
      );
    case "phone":
      return (
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <div className="card">
              ...
              <div className="mt-4 flex h-28 items-center justify-center rounded-xl border border-dashed border-wood/30 bg-wood/5">
                <div className="text-center">
                  ...
                  <p className="mt-1 text-[11px] text-ink-muted">{consultPaths[1].note}</p>
                </div>
              </div>
            </div>
            ...
            <div className="card">
              ...
              <div className="mt-4 rounded-xl bg-cream-50 p-4">
                ...
                <p className="mt-1 text-xs text-ink-muted">{consultPaths[2].note}</p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-12 rounded-2xl border border-forest/10 bg-white p-6 sm:p-8">
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {prepChecklist.map((item) => (
                <div key={item.title} className="rounded-xl bg-cream-50 p-4 transition-colors hover:bg-wood/5">
                  <div className="flex items-center gap-2 text-forest">
                    <ChecklistIcon name={item.icon} />
