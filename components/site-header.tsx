    ...
    <header className="sticky top-0 z-40 border-b border-forest/10 bg-cream/85 backdrop-blur-md supports-[backdrop-filter]:bg-cream/75">
      <div className="container-page flex h-16 items-center justify-between gap-4">
        ...
        >
          <span
            aria-hidden="true"
            className="brand-mark inline-flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-forest to-forest-800 text-base font-bold text-cream shadow-soft ring-1 ring-wood/20"
          >
            晟
          </span>
        ...
        <nav className="hidden items-center gap-0.5 lg:flex" aria-label="主导航">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="nav-link rounded-lg px-3 py-2 text-sm font-medium text-ink-soft transition-all hover:bg-forest/5 hover:text-forest"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-2">
          <Link
            href="/contact"
            className="hidden rounded-xl bg-forest px-4 py-2 text-sm font-medium text-cream shadow-soft transition-all hover:bg-forest-800 hover:shadow-card sm:inline-flex"
          >
            预约量房
          </Link>
      ...
      {open && (
        ...
        >
          <div className="container-page flex flex-col py-2">
            ...
            <Link
              href="/contact"
              className="mt-2 rounded-xl bg-forest px-3 py-3 text-center text-sm font-medium text-cream"
              onClick={() => setOpen(false)}
            >
              预约量房
            </Link>
